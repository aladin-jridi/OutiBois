	...
	$.JQPlugin.createPlugin({
				...
				if (a.options.serverSync) {
					...
					if (b[2] == null) {
						var c = $.isFunction(a.options.serverSync)
							...
							: null;
						b[2] =
							(c ? new Date().getTime() - c.getTime() : 0) - b[1];
					}
					if (a._since) {
						a._since.setMilliseconds(
							a._since.getMilliseconds() + b[2]
						);
					}
		...
		_adjustSettings: function (a, b, c) {
			...
			if (c || (!c && b._until == null && b._since == null)) {
				...
				if (b._since != null) {
					b._since = this.UTCDate(
						h,
						this._determineTime(b._since, null)
					);
					if (b._since && e) {
						b._since.setMilliseconds(
							b._since.getMilliseconds() + e
						);
					}
				}
				b._until = this.UTCDate(
					h,
					this._determineTime(b.options.until, f)
				);
				if (e) {
		...
		_generateHTML: function (c) {
			...
			var k = c.options.compact
				? c.options.compactLabels
				: c.options.labels;
		...
		_calculatePeriods: function (c, d, e, f) {
			...
			if (d[Y] || d[O]) {
				...
				var k =
					g.getDate() == f.getDate() ||
					(g.getDate() >= Math.min(i, j) &&
						f.getDate() >= Math.min(i, j));
				var l = function (a) {
					return (
						(a.getHours() * 60 + a.getMinutes()) * 60 +
						a.getSeconds()
					);
				};
				var m = Math.max(
					...
					(g.getFullYear() - f.getFullYear()) * 12 +
						g.getMonth() -
						f.getMonth() +
						((g.getDate() < f.getDate() && !k) || (k && l(g) < l(f))
							? -1
							: 0)
				);
	...
	e.infinitescroll.prototype = {
		...
		_create: function (r, i) {
			...
			if (s.pixelsFromNavToBottom === t) {
				...
				this._debug(
					"pixelsFromNavToBottom: " + s.pixelsFromNavToBottom
				);
			}
			var f = this;
			s.loading.start =
			...
			s.callback = function (n, r, u) {
				if (!!s.behavior && n["_callback_" + s.behavior] !== t) {
					n["_callback_" + s.behavior].call(
						e(s.contentSelector)[0],
						r,
						u
					);
				}
				...
				return this.reverting
					...
					: (this._refreshItems(t),
					  e(t.target)
							.parents()
							.each(function () {
								return e.data(this, a.widgetName + "-item") ===
									a
									? ((s = e(this)), !1)
									: void 0;
							}),
