...
$(document).ready(
	(function () {
		...
		$module
			.find("input.form-control", ".btn", ".cancel")
			.on("click", function (e) {
				e.stopPropagation();
			});
		...
		function chimpCallback(resp) {
			if (resp.result === "success") {
				$(".subscribe-alert")
					.html(
						'<h5 class="alert alert-success">' + resp.msg + "</h5>"
					)
					.fadeIn(1000);
				...
				$(".subscribe-alert")
					.html(
						'<h5 class="alert alert-danger">' + resp.msg + "</h5>"
					)
					.fadeIn(1000);
			}
		}
		...
		popQuoteForm.validate({
			...
			submitHandler: function (popQuoteForm) {
				...
				$.ajax({
					...
					timeout: 20000,
					success: function (msg) {
						$(popQuoteForm).fadeOut(
							(500,
							function () {
								$(popQuoteForm)
									.html(
										'<div class="alert alert-success text-center" role="alert"><strong>Thank you.<br/> We will contact you shortly.</strong></div>'
									)
									.fadeIn();
							})
						);
